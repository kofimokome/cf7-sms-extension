...
class CF7SmsExtension {
	...
	public function add_main_menu() {
		...
		$menu_page = new MenuPage( 'CF7 SMS Extension', 'CF7 SMS Extension', 'read', 'kmcf7se-sms-extension-options', 'dashicons-tickets-alt', null, array(
			$this,
			'dashboard_view'
		) );

		$settings_page = new SubMenuPage( $menu_page->get_menu_slug(), 'Options', 'Options', 'manage_options', 'kmcf7se-sms-extension-options', array(
			$this,
			'settings_view'
		), true );
		$settings_page->add_tab( 'settings', 'Basic Settings', array(
			$this,
			'status_tab_view'
		), array( 'tab' => 'settings' ) );
	...
	public function before_send_email( $form, &$abort, $submission ) {
		...
		if ( $send_sms_to_me && strlen( $your_numbers ) > 0 ) {
			...
			foreach ( $your_numbers as $your_number ) {
				...
				if ( ! $this->send_sms( $your_number, $your_message, true ) ) {
					// $abort = true;
				}
			}
		}
	...
	public static function send_sms( $to, $message, $skip_error = false ) {
	...
	public function editor_panel_sms( $post, $args = '' ) {
        ...
        <fieldset>
            <legend>
                Your Phone Number: (<strong>leave blank if you do not want to receive a message</strong>) <br>
